module XcodeResultBundleProcessor
  describe ElementSnapshot do
    it do
      ...
      expect(element['$class']).to eq('XCElementSnapshot')
      expect(element.accessibilityElement['payload']).to eq({'pid' => 59497, 'uid.elementOrHash' => 0, 'uid.elementID' => 1})
      ...
      expect(element.children.first['children'].first['children'].first['additionalAttributes'][5004]).to eq('UINavigationBar')
    end
  end
end